...
import { getProgramCategoryCreditProgress } from '../../../src/features/study-plan/credit-progress';
...
const CONVERTED_MAJOR_BRANCH_COUNTS: Record<string, number> = {
  'khoa-cntt/khoa-hoc-may-tinh': 5,
  'khoa-khoa-hoc-va-cong-nghe-vat-lieu/cong-nghe-vat-lieu': 4,
  'khoa-toan/toan-hoc': 4,
  'khoa-toan/toan-tin': 3,
  'khoa-toan/cu-nhan-tai-nang': 4,
  'khoa-dia-chat/dia-chat-hoc': 6,
  'khoa-ly/vat-ly-hoc': 7,
  'khoa-ly/hai-duong-hoc': 4,
  'khoa-ly/ky-thuat-hat-nhan': 3,
  'khoa-moi-truong/quan-ly-tai-nguyen-va-moi-truong': 3,
  'khoa-dien-tu-vien-thong/ky-thuat-dien-tu-vien-thong': 3,
};
...
describe('K23 academic programs', () => {
  it('registers exactly 9 faculties and 29 canonical programs without cohort fallback', () => {
    ...
    faculties.flatMap((faculty) => faculty.majors).forEach((major) => {
      expect(major.campusIds.length, `${major.id} must have at least one campus`).toBeGreaterThan(0);
      expect(major.tuitionProfileId, `${major.id} must have a tuition profile`).toBe('tuition-cs2');
    });
  ...
  it('calculates totals for K23 programs with alternative MAJOR branches', async () => {
    const expectedTotals = [
      ['khoa-ly', 'hai-duong-hoc', 129],
      ['khoa-ly', 'ky-thuat-hat-nhan', 132],
      ['khoa-moi-truong', 'khoa-hoc-moi-truong', 135],
    ] as const;

    for (const [facultyId, majorId, total] of expectedTotals) {
      const { categories } = await loadCohortData(facultyId, majorId, 'k23');
      expect(getProgramRequiredCredits(categories), `${facultyId}/${majorId}`).toBe(total);
    }
  });

  it('counts a shared K23 course independently in every matching MAJOR branch', async () => {
    const { categories } = await loadCohortData(
      'khoa-khoa-hoc-va-cong-nghe-vat-lieu',
      'cong-nghe-vat-lieu',
      'k23',
    );
    const major = (categories as Record<string, any>).MAJOR;
    const sharedCourseId = 'MST10112';
    const passedCourse = {
      course_id: sharedCourseId,
      course_name_vi: sharedCourseId,
      credits: 3,
      status: 'passed',
    };
    const processedMajor = {
      ...major,
      breakdown: Object.fromEntries(
        Object.entries<any>(major.breakdown).map(([key, branch]) => [
          key,
          {
            ...branch,
            allCoursesData: branch.courses.includes(sharedCourseId) ? [passedCourse] : [],
            coursesData: branch.courses.includes(sharedCourseId) ? [passedCourse] : [],
          },
        ]),
      ),
    };

    const progress = getProgramCategoryCreditProgress({ MAJOR: processedMajor }, new Set());
    Object.keys(processedMajor.breakdown).forEach((key) => {
      expect(progress[`MAJOR.${key}`].display).toEqual({ earnedCredits: 3, plannedCredits: 0 });
    });
    expect(progress.MAJOR.display).toEqual({ earnedCredits: 3, plannedCredits: 0 });
  });
  ...
  it.each(PROGRAMS)('$facultyId/$majorId exposes valid app-reachable data', async ({
    ...
    const categoryMap = categories as Record<string, any>;
    const major = categoryMap.MAJOR;
    expect(major?.options, `${facultyId}/${majorId}.MAJOR must use breakdown`).toBeUndefined();

    const convertedBranchCount = CONVERTED_MAJOR_BRANCH_COUNTS[`${facultyId}/${majorId}`];
    if (convertedBranchCount) {
      const majorBranchKeys = Object.keys(major?.breakdown ?? {});
      expect(majorBranchKeys).toHaveLength(convertedBranchCount);
      expect(majorBranchKeys.every((key) => key.startsWith('MAJOR_'))).toBe(true);
    }

    const generalEducation = categoryMap.GENERAL_EDUCATION;
    const generalEducationKeys = [
      'GENERAL_IT',
      'GENERAL_ENGLISH',
      'GENERAL_PE',
      'GENERAL_DEFENSE',
    ];
    generalEducationKeys.forEach((key) => {
      expect(categoryMap[key], `${facultyId}/${majorId}.${key} must not be top-level`).toBeUndefined();
      expect(
        generalEducation?.breakdown?.[key],
        `${facultyId}/${majorId}.${key} must belong to GENERAL_EDUCATION`,
      ).toBeDefined();
    });

    const graduation = categoryMap.GRADUATION
      ?? categoryMap.PROFESSIONAL_EDUCATION?.breakdown?.GRADUATION;
    expect(graduation, `${facultyId}/${majorId} must expose graduation requirements`).toBeDefined();
    expect(
      graduation.breakdown,
      `${facultyId}/${majorId}.GRADUATION must not be split by specialization`,
    ).toBeUndefined();
