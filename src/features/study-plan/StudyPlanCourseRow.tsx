    ...
    return (
        <div className="group">
            <div
                role="button"
                tabIndex={0}
                aria-label={`Chi tiết ${course.course_name_vi}`}
                onKeyDown={event => { if (event.target === event.currentTarget && (event.key === 'Enter' || event.key === ' ')) { event.preventDefault(); if (!onOpenMobilePlanner(course, rootCompleted)) setShowDetails(value => !value); } }}
                draggable={!isLocked}
                onClick={() => {
