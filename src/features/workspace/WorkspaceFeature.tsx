import { Bookmark, Calculator, Code2, Database, FlaskConical, HardDrive, ShieldCheck } from 'lucide-react';
...
import { WorkspaceAnalyticsFeature } from './components/WorkspaceAnalyticsFeature';

type WorkspaceTab = 'lab' | 'data' | 'workload' | 'storage' | 'analytics';
...
const WORKSPACE_TABS: NavTab<WorkspaceTab>[] = [
    ...
    { id: 'analytics', label: 'SQL analytics', description: 'Truy vấn snapshot local', icon: Code2 },
];
...
export function WorkspaceFeature() {
    ...
    const isAnalyticsWorkspace = location.pathname.startsWith('/ad/analytics');
        ...
        >
            <div className="min-w-0">
                <NavigationBar
                    ariaLabel="Workspace"
                    tabs={WORKSPACE_TABS}
                    activeTab={isDataWorkspace ? 'data' : isWorkloadWorkspace ? 'workload' : isStorageWorkspace ? 'storage' : isAnalyticsWorkspace ? 'analytics' : 'lab'}
                    setActiveTab={(tab) => navigate(tab === 'data' ? '/ad/data' : tab === 'workload' ? '/ad/workload' : tab === 'storage' ? '/ad/storage' : tab === 'analytics' ? '/ad/analytics' : '/ad/lab')}
                />
